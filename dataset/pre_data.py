...
class Dataset():
    def __init__(self, input_size, root, dataset_path, mode='train'):
        ...
        train_img_path = test_img_path = dataset_path        

        train_label_file = open(os.path.join(self.root, 'cls3_train.txt'))
        # val_label_file = open(os.path.join(self.root,'val.txt'))
        test_label_file = open(os.path.join(self.root, 'cls3_test.txt'))

        train_img_label = []
        # val_img_label = []
        test_img_label = []
        for line in train_label_file:
            train_img_label.append([os.path.join(train_img_path,line[:-1].split(' ')[0]), int(line[:-1].split(' ')[-2]), int(line[:-1].split(' ')[-1])])

        # for line in val_label_file:
        #     val_img_label.append([os.path.join(val_img_path,line[:-1].split(' ')[0]), int(line[:-1].split(' ')[1])])
        for line in test_label_file:
            test_img_label.append([os.path.join(test_img_path,line[:-1].split(' ')[0]), int(line[:-1].split(' ')[-2]), int(line[:-1].split(' ')[-1])])
    ...
    def __getitem__(self, index):
        if self.mode == 'train':
            img, target = imageio.imread(self.train_img_label[index][0]), (self.train_img_label[index][1], self.train_img_label[index][2])
